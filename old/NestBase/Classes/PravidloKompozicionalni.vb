Public Class PravidloKompozicionalni
    Inherits Pravidlo
    ...
    Public Sub New(ByVal iEnvironment As Environment, ByVal iLanguage As Enums.enmLanguage)
        ...
        Me.mTyp = Enums.enmTypPravidla.enmKompozicionalni
        mPredpoklad = Nothing
        mKontext = Nothing
    End Sub
...
#Region "Xml"
    Public Overrides Function LoadFromXML(ByVal xmlElement As Xml.XmlElement, ByVal defaultVaha As Interval, ByVal prahPlatnostiKontextu As Double, ByVal prahPlatnostiPredpokladu As Double, ByVal Atributy As Collection, ByVal Kontexty As Collection, ByVal rozsahVah As Long) As Boolean
        ...
        Try
            ...
            If Not mPredpoklad.LoadFromXML(predpokladElement, defaultVaha, Atributy, rozsahVah) Then
                Return Me.AppendLastError(mPredpoklad.LastError, GetText("PravidloKompozicionalni_pravidlo") & Me.mId)
            End If

            Return True
        Catch ex As Exception
            Return Me.SetError(ex, GetText("PravidloKompozicionalni_Chyba_pri_nacitani_kompozicionalniho_pravidla_z_XML"))
        End Try
    ...
    Public Overrides Function SaveToXML(ByRef writer As Xml.XmlTextWriter, ByVal rozsahVah As Long) As Boolean
        Try
            writer.WriteStartElement("compositional_rule")
            ...
            Return True
        Catch ex As Exception
            Return Me.SetError(ex, GetText("PravidloKompozicionalni_Chyba_pri_SaveToXML_v_pravidlu") & Me.mId)
        End Try
    End Function
#End Region
    ...
    Public Overrides Function PropojeniBaze() As Boolean
        Try
            ...
            Return True
        Catch ex As Exception
            Return Me.SetError(ex, GetText("PravidloKompozicionalni_Chyba_pri_propojovani_kompozicionalniho_pravidla") & Me.mId)
        End Try
    End Function
    ...
    Public Overrides Function GetVypis(ByRef text As String, ByVal typVypisu As Enums.enmTypVypisu, ByVal rozsahVah As Long) As Boolean
        Helper.VypisAddLine(text, Enums.TxtEnmTypPravidla(Enums.enmTypPravidla.enmKompozicionalni, Me.GetResourceManager), typVypisu, Enums.enmTypRadkyVypisu.enmNadpis2)
        ...
        Try
            ...
            Return True
        Catch ex As Exception
            Return Me.SetError(ex, GetText("pravidloKompozicionalni_Chyba_pri_tvorbe_vypisu_kompozicionalniho_pravidla") & Me.mId)
        End Try
    End Function
    ...
    Public Overrides Function AplikujBackward(ByRef uspesneAplikovano As Boolean, ByVal neurcitost As Neurcitost, ByVal aktSeznamDotazu As Collection, ByVal atributy As Collection, ByVal bezdotazu As Boolean) As Boolean
        Try
            ...
            If Not Me.mKontext Is Nothing Then
                'zjistim, jeslti predpoklad nelze vyhodnotit a neni zaporny (pak by nemelo smysl kontext vyhodnocovat)
                Dim vyhodnocovatKontext As Boolean = True
                Dim vyhodnoceniPredpokladuPom As Boolean
                If Not Me.mPredpoklad.VyhodnotBackward(vyhodnoceniPredpokladuPom, neurcitost, Nothing, atributy, True) Then
                    Return Me.SetError(mPredpoklad.LastError)
                End If
                If vyhodnoceniPredpokladuPom Then
                    If mPredpoklad.Vaha.MaxHodnota <= 0 Then
                        vyhodnocovatKontext = False
                    End If
                End If
                If vyhodnocovatKontext Then
                    ...
                    'kontext nepresah prah platnosti
                    Dim zav As Zaver
                    ...
                    For Each zav In Me.mZavery
                        zav.Ctr.SetValue(neurcitost.CTR(mKontext.Vaha, zav.Ctr))
                    Next
            ...
            For Each zaver In mZavery
                ...
                pomocnyInterval = neurcitost.CTR(mPredpoklad.Vaha, zaver.Ctr)
            ...
            Return True
        Catch ex As Exception
            Return Me.SetError(ex, GetText("pravidloKompozicionalni_Chyba_pri_aplikovani_kompozicionalniho_pravidla_backward") & Me.mId)
        End Try
    End Function

    Public Function PridejPravidloZeStringu(ByVal pravidloStr As String, ByVal DefaultVaha As Interval, ByVal atributy As Collection, ByVal kontexty As Collection) As Boolean
        'kontext: IF (A and b)or(c) THEN d(0.8)
